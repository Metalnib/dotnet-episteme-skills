...
switch ($Mode) {
    ...
    "full" {
        ...
        git --no-pager diff "$MergeBase...HEAD"
    }
    "cs" {
        Write-Host "=== C# files changed vs $TargetBranch ===" -ForegroundColor Cyan
        $files = git --no-pager diff --name-only "$MergeBase...HEAD" | Where-Object { $_ -match '\.(cs|csproj|sln)$' }
