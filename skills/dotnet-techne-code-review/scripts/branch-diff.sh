...
# Find merge base
MERGE_BASE=$(git --no-pager merge-base HEAD "$TARGET_BRANCH" 2>/dev/null || echo "$TARGET_BRANCH")
...
case "$MODE" in
    ...
    --cs)
        ...
        git --no-pager diff --name-only "$MERGE_BASE"...HEAD | grep -E '\.(cs|csproj|sln)$' || echo "(no C# files changed)"
        ;;
    *)
